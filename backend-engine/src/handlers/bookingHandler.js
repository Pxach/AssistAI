...
export async function handleBooking(message, language, bookingState = {}, history = [], senderPhone = "", botPhone = "", ioContext = {}) {
  ...
  const prompt = `
    ...
    Rules & Validations:
    - CRITICAL SCHEDULING RULE: You MUST cross-reference the user's requested time with the specific working_hours of the assigned professional in the provided JSON data. If the requested time is BEFORE their shift starts or AFTER their shift ends, you are STRICTLY FORBIDDEN from booking the appointment. You must apologize, state the professional's actual working hours, and ask the user to choose a valid time. Do not attempt to accommodate requests outside these hours under any circumstances.
    - STATE MANAGEMENT RULE FOR OUT-OF-HOURS TIME: If the user requests a time outside the professional's working hours, you must verbally reject it AND you are strictly forbidden from saving that invalid time to the extracted state. You must enforce appointment_time: null (or retain its previous empty state) in your JSON output. Do not populate the time field until the user provides a valid time that strictly falls within the schedule.
